...
def get_current_robot_type() :
    rt = 1003 ## TEST:1003:cue, otherwise can get it from ??serial number
    return rt


def parse_sensor0_packet(p):
    assert(len(p) == 20)
    res = {}
    res["tm"] = 0
    b = p[1] | (p[0] & 0x0f) << 8
    ...
    if (p[19] < 0x80) :
        for ofs in range(0, 5, 4) : # first run: 0, second run: -4 
            ...
            if evt in [0,1,4] :
                ...
                if p[19] == 2: print("DBG|3005 ofs:", ofs, "p[19]", p[19], "p[7]:", p[7], "3006:", res["3006"])
                pass
...
def parse_sensor1_packet(p):
    ...
    ## ?? watermark ==> needs sensor0 data (byte[16], byte[19] == 1)

    return res

def match_sensor1_data(d1, d2):
    matched = False
    for k in d1 :
        kd1 = d1[k]
        kd2 = d2[k]
        for kv in kd1 :
            if type(kd1[kv]) == list:
                ...
                if not abs(float(kd1[kv]) - float(kd2[kv])) < eps :
                    print("DBG|s1: not matched:", k, kv, kd1[kv], "expected:", kd2[kv])
                    return matched
...
def test_sensor():
    ...
    data = [ b'\x10\x00\xeeN\xff0\xfb\x01\x00\x00\x000d\xe2\xe1\x08\x04\xdd\r\x02', None]
    packets = [ bytearray(data[0]), bytearray(data[1]) if data[1] is not None else None ]
    s0_data = parse_sensor0_packet(packets[0])
    print("DBG|s0:", s0_data)
    
    #if not match_sensor0_data(s1_data, json.loads(jstr)):
    #    print("DBG|s0: not matched")

    #s1_data = parse_sensor1_packet(packets[1])
    ...
    with open(fname, "r") as f:
        ...
        for line in f :
            ...
            if sid == 0x17 :
                ...
                s0 = parse_sensor0_packet(packets[0])
                print("{}:{}".format(ln1, s0))
            elif sid == 0x1a :
                ln2 = ln
                packets[1] = p[:]
                if packets[0] and packets[0][0] == (packets[1][0] & 0xf0) : ## matching seq
                     ...
                     s0 = parse_sensor0_packet(packets[0])
                     print("{}:{}".format(ln2, s0))
                     s1 = parse_sensor1_packet(packets[1])
                     #print("{}:{}".format(ln2, s1))
                     continue
