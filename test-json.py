...
def test_json2Packets():
    ...
    assert_json_eq_hex({"108":{"1":-2}}, ["31 02 00 00"])
    ...
    assert_json_eq_hex({"206":{"time":-81.93,"hold":True}}, ["71 00 00 5f ff"])
    ...
    assert_json_eq_hex({"213":{}}, ["75 00"])  ## message:"malformed json for component 213: missing prcnt"
    ...
    assert_json_eq_hex({"214":{}}, ["76 00"])  ## message:"malformed json for component 214: missing prcnt"
    ...
    assert_json_eq_hex({"304":{"perc1":2,"perc2":4,"duration":100,"tone1":{"type":3,"freq":300,"phase":30},"tone2":{"type":2,"freq":400},"tone4":{"freq":600}}}, ["21 00 60 64 80 0b b8 1e 0f a0 00 00 00 00 17 70 00 00 02 04"])
    assert_json_eq_hex({"304":{"type":3,"tone1":{"freq":300},"tone2":{"freq":7040},"tone3":{"freq":500},"tone4":{"freq":600}}}, ["21 00 00 00 00 0b b8 00 13 00 00 13 88 00 17 70 00 00 00 00"])  ## freq: wraps around 0xffff instead of max-limited
    assert_json_eq_hex({"304":{"perc1":260,"perc2":360,"duration":2000,"tone1":{"type":3,"freq":4400,"phase":30},"tone2":{"type":2,"freq":5400}}}, ["21 00 67 d0 80 ab e0 1e d2 f0 00 00 00 00 00 00 00 11 04 68"])
    assert_json_eq_hex({"304":{"perc1":-2048}}, ["21 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 80 00 00"])
    assert_json_eq_hex({"304":{"perc1":2048}}, ["21 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 80 00 00"])
    assert_json_eq_hex({"304":{"perc1":2047}}, ["21 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 70 ff 00"])
    assert_json_eq_hex({"304":{"perc2":-2048}}, ["21 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 08 00 00"])
    assert_json_eq_hex({"304":{"perc2":2048}}, ["21 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 08 00 00"])
    assert_json_eq_hex({"304":{"perc2":2047}}, ["21 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 07 00 ff"])
    ...
    if (p[19] < 0x80) :
        ...
        if evt == 0x81 :   ## 3009 beacon_v2 (p[12,13,14,15,16,17,18])
            ...
            if rt == 0 :
                ...
                d["rbtType"] = 1003  ## cue
            elif rt == 3 and get_current_robot_type() in [1001, 1002]:  ## ?? dash/dot receiving from cue
                d["rbtType"] = 1003  ## cue
            else :
...
#13410:[f0 00 18 88 0f 40 01 01 00 00 00 30 fa 36 d7 08 00 00 00 00]:[f0 89 00 01 0f ff ff ff ff 00 00 00 19 02 00 00 fe ff 04 00]:{'tm': 0, '2003': {'x': 0.0234489, 'y': -0.117245, 'z': 1.00147}, '5101': {'data': [240, 0, 24, 136, 15, 64, 1, 1, 0, 0, 0, 48, 250, 54, 215, 8, 0, 0, 0, 0]}, '1000': {'s': 0}, '1001': {'s': 0}, '1002': {'s': 0}, '1003': {'s': 0}, '3005': {'amp': 0.00392157, 'mictconf': 102, 'mictdir': -1.74533, 'clap': 0}, '4001': {'flag': 0}, '4002': {'flag': 0}, '4003': {'flag': 0}, '4006': {'flag': 0}, '5102': {'data': [240, 137, 0, 1, 15, 255, 255, 255, 255, 0, 0, 0, 25, 2, 0, 0, 254, 255, 4, 0]}, '2004': {'r': -0.00426314, 'p': 0.00426314, 'y': 0}, '3000': {'cm': 2.5, 'refl': 255}, '3001': {'cm': 2.5, 'refl': 255}, '3002': {'cm': 5, 'refl': 255}, '3003': {'cm': 0}, '3004': {'cm': -0.0411025}, '2000': {'degree': 0}, '2001': {'degree': -1.14592}, '2002': {'x': 0, 'y': 0, 'degree': 30.7678}}
    ...
    data = [ bytearray([192, 0, 36, 15, 0, 64, 28, 0, 0, 0, 0, 48, 163, 2, 0, 0, 96, 8, 0, 129]), None ]
    data = [ bytearray([128, 0, 29, 234, 15, 64, 9, 0, 0, 0, 0, 48, 104, 128, 0, 0, 96, 2, 0, 129]), None ]
    packets = [ bytearray(data[0]), bytearray(data[1]) if data[1] is not None else None ]
    if packets[0] :
